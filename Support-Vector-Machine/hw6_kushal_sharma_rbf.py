...
def plot_nonlinsep_points(points, target, alpha=0, index=0, b=0, points_sv=0):
    ...
    for j in range(len(X)):
        s = 0
        for i in index:
            s += alpha[i] * target[i] * polynomial_kernel(points[i], X[j])
            # RBF
            # s += alpha[i] * target[i] * rbf_kernel(points[i], X[j])
        y_predict[j] = s
...
def svm_nonlinsep(nonlinsep_points, nonlinsep_labels, outfile):
    ...
    for i in range(len(nonlinsep_labels)):
        for j in range(len(nonlinsep_labels)):
            kernel_function = polynomial_kernel(nonlinsep_points[i], nonlinsep_points[j])
            nonlinsep_q_matrix_iter[i][j] = nonlinsep_labels[i] * nonlinsep_labels[j] * kernel_function
    print("q_iter:", nonlinsep_q_matrix_iter)

    nonlinsep_kernel = polynomial_kernel(nonlinsep_points, nonlinsep_points)
    ...
    # for i in range(len(nonlinsep_labels)):
    #     for j in range(len(nonlinsep_labels)):
    #         kernel_function = rbf_kernel(nonlinsep_points[i], nonlinsep_points[j])
    #         nonlinsep_q_matrix_iter[i][j] = nonlinsep_labels[i] * nonlinsep_labels[j] * kernel_function
    # print("q_iter:", nonlinsep_q_matrix_iter.shape, nonlinsep_q_matrix_iter)
    #
    # nonlinsep_kernel = rbf_kernel(nonlinsep_points, nonlinsep_points)
    # nonlinsep_q_matrix = np.outer(nonlinsep_labels, nonlinsep_labels) * nonlinsep_kernel
    # print("q_matrix:", nonlinsep_q_matrix.shape, nonlinsep_q_matrix)

    # alpha, index = quadratic_prog_solver(nonlinsep_q_matrix, nonlinsep_labels, N)
    alpha, index = quadratic_prog_solver(nonlinsep_q_matrix_iter, nonlinsep_labels, N)
    ...
    3. 
            exp(-gamma * ((x-x')**2)) = exp(-gamma * (x**2 + x'**2 - 2xx'))
            = exp(-gamma * ((x1**2 + x2**2) + (x1'**2 + x2'**2) - 2 * (x1x1' + x2x2')))
    '''
    ...
    for i in index:
        ...
        # RBF
        # bias -= alpha[i] * nonlinsep_labels[i] * rbf_kernel(nonlinsep_points[i], nonlinsep_points[index[0]].T)
    ...
    for j in range(len(nonlinsep_points)):
        s = 0
        for i in index:
            s += alpha[i] * nonlinsep_labels[i] * polynomial_kernel(nonlinsep_points[i], nonlinsep_points[j])
            # RBF
            # s += alpha[i] * nonlinsep_labels[i] * rbf_kernel(nonlinsep_points[i], nonlinsep_points[j])
        y_predict[j] = np.sign(s + bias)
    ...
    outfile.write('Support vectors indices: ' + str(index) + '\n')
    outfile.write('Support lagrangian multiplier: ' + str(alpha[index]) + '\n')
    outfile.write('Support Vectors:' + str(len(support_vectors)) + '\n' + str(support_vectors) + '\n')
    outfile.write('bias: ' + str(bias) + '\n')
    outfile.write('Equation of line: ' + str(equation_line) + '= 0\n')
    outfile.write('Accuracy: ' + str((N - misclassified) * 100/N) + '%\n')
    outfile.write('=============================================\n\n')
